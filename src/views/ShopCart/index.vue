  ...
  <div class="cart">
        ...
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          <li class="cart-list-con1">
            <input
              ...
              @change="changeCartState(cart.skuId, $event)"
            />
          </li>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllCheck && cartInfoList.length >0" @click="changeAllCartState($event)" />
        <span>全选</span>
      </div>
      <div class="option">
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      return this.cartInfoList
        ...
        .reduce((pre, cur) => (pre += cur.skuNum * cur.cartPrice), 0);
    },
    isAllCheck() {
  ...
  methods: {
    ...
    handle: throttle (async function (type, disNum, cart) {
      switch (type) {
        ...
        case "minus":
          ...
          break;
        case "change":
          disNum = parseInt(disNum) < 1 ? 0 : parseInt(disNum) - cart.skuNum;
          break;
      }
      try {
    ...
    async deleteCartList(id) {
      await this.$store.dispatch("deleteCarList", id);
      this.getDate();
    },
    async changeCartState(id, event) {
      try {
        let isChecked = event.target.checked ? "1" : "0";
        await this.$store.dispatch("changeCartState", {
          skuID: id,
          isChecked
        });
        this.getDate();
    ...
    async changeAllCartState(event){
      try {
        let isChecked = event.target.checked ?'1':'0'
        await this.$store.dispatch('changeAllCartState',isChecked)
        this.getDate()
      } catch (error) {
        console.log(error.message);
      }
